...
def amplifier(phase, input, core):
    computer = IntCodeComputer()
    computer.load_core(core)
    computer.set_input_buffer([input, phase])
    computer.run()
    return computer.output_buffer[0]


def amplification_circuit(phase_vector, core):
    signal = 0
    for phase in phase_vector:
        signal = amplifier(phase, signal, core)
    return signal
...
def main():
    ...
    max_sig = None
    for phase_vector in permute([0, 1, 2, 3, 4]):
        sig = amplification_circuit(phase_vector, core)
        if max_sig is None:
            max_sig = sig
        else:
